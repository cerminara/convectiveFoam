...
int main(int argc, char *argv[])
{
    ...
    forAll(timeDirs, timeI)
    {
        ...
        if (phiHeader.headerOk())
        {
            ...
            pointScalarField streamFunctionXZ
            (
                IOobject
                (
                    "streamFunctionXZ",
            ...
            volScalarField streamFunctionXZV
                 (
                      IOobject
                      (
                          "streamFunctionXZV",
            ...
            do
            {
                ...
                forAll(patches, patchi)
                {
				    ...
				    forAll(zeroPoints, pointi)
                                    {
                                        streamFunctionXZ[zeroPoints[pointi]] = 0;
                ...
                if (!found)
                {
                    ...
                    forAll(c, cI)
                    {
                        ...
                        if (found)
                        {
                            forAll(zeroPoints, pointi)
                            {
                                streamFunctionXZ[zeroPoints[pointi]] = 0.0;
                                visitedPoint[zeroPoints[pointi]] = 1;
                                nVisited++;
                            }
                     ...
                     )
                     {
                         ...
                         forAll(curBPoints, pointi)
                         {
                             ...
                             if (visitedPoint[curBPoints[pointi]] == 1)
                             {
                                 ...
                                 currentBStream =
                                     streamFunctionXZ[curBPoints[pointi]];
                                 currentBStreamPoint =
                     ...
                     for (label facei=0; facei<nInternalFaces; facei++)
                     {
                         ...
                         forAll(curPoints, pointi)
                         {
                             ...
                             if (visitedPoint[curPoints[pointi]] == 1)
                             {
                                 // The point has been visited
                                 currentStream =
                                     streamFunctionXZ[curPoints[pointi]];
                                 currentStreamPoint =
            ...
            streamFunctionXZ /= thickness;


            streamFunctionXZV = dimensionedScalar("zero", phi.dimensions(), 0.0);
            weight = 0.0;
            const labelListList& cellPoints = streamFunctionXZV.mesh().cellPoints();
            forAll(streamFunctionXZV, cellI)
            {
                const labelList& curCellPoints = cellPoints[cellI];
                forAll(curCellPoints, cellPointI)
                {
                    streamFunctionXZV[cellI] += 
                        streamFunctionXZ[curCellPoints[cellPointI]];
                    weight[cellI] += 1.0;
                }
            }
 
            streamFunctionXZV /= (weight + 1.0);
            streamFunctionXZV.boundaryFieldRef() = 0.0;
            streamFunctionXZV.write();

 
            streamFunctionXZ.boundaryFieldRef() = 0.0;
